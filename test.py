...
def sample_and_test(rank, gpu, args):
    ...
    ckpt = torch.load(
        f"{args.exp_root_dir}/{args.exp}/model_{args.epoch_id}.pth",
        map_location=device,
    )
    ...
    save_dir = f"./consistent_generated_samples/{args.dataset}/exp{args.exp}_ep{args.epoch_id}_m{args.method}"
    if args.method in FIXER_SOLVER:
        save_dir += f"_s{args.num_steps}"
    if args.stochastic:
        save_dir += f"_stochastic{args.beta}"
    ...
    def run_sampling(num_samples, generator, cls_index=None, return_traj=False):
        x = generator.randn(
            num_samples, 4, args.image_size // 8, args.image_size // 8,
        ).to(device)
        if args.num_classes in [None, 1]:
            ...
            if cls_index is None:
                y = generator.randint(
                    0, args.num_classes, (num_samples,), device=device,
                )
            else:
        ...
        if args.stochastic:
            traj, x0_list = flow.sample_ode_generative_stochastic(
                x, model_kwargs=model_kwargs, beta=args.beta, solver=args.method,
            )
        else:
            traj, x0_list = flow.sample_ode_generative(
                x, model_kwargs=model_kwargs, solver=args.method,
            )
        fake_sample = traj[-1]
        ...
        if return_traj:
            traj = [
                first_stage_model.decode(x / args.scale_factor).sample for x in traj
            ]
            traj = torch.cat(traj, dim=0)
            x0_list = [
                first_stage_model.decode(x / args.scale_factor).sample for x in x0_list
            ]
    ...
    if args.measure_time:
        ...
        print(f"Inference time: {mean_syn:.2f}+/-{std_syn:.2f}ms")
        exit(0)
    ...
    if args.compute_fid:
        ...
        total_samples = int(
            math.ceil(args.n_sample / global_batch_size) * global_batch_size,
        )
        if rank == 0:
            print(f"Total number of images that will be sampled: {total_samples}")
        assert total_samples % args.world_size == 0, (
        ...
        if rank == 0:
            ...
            print(f"FID = {fid}, beta = {args.beta}")
            with open(args.output_log, "a") as f:
                f.write(
                    f"Epoch = {args.epoch_id}, FID = {fid}, beta = {args.beta}\n",
                )
        dist.barrier()
        ...
        print("Inference")
        with torch.no_grad():
            fake_image, traj, x0_seq = run_sampling(
                args.batch_size, generator, return_traj=True,
            )
        fake_image = torch.clamp(to_range_0_1(fake_image), 0, 1)
        if not args.use_karras_samplers:
            save_path = f"samples_{args.dataset}_{args.method}_{args.atol}_{args.rtol}"
        else:
            save_path = f"samples_{args.dataset}_{args.method}_{args.num_steps}"
        if args.stochastic:
        ...
        if args.plot_traj:
            torchvision.utils.save_image(
                x0_seq, save_path.split(".")[0] + "_x0pred.jpg", normalize=True, nrow=4,
            )
            torchvision.utils.save_image(
                traj, save_path.split(".")[0] + "_traj.jpg", normalize=True, nrow=4,
            )
        print(f"Samples are saved at {save_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="plot the whole denoising trajectory",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="use a modified ODE function with a control of stochasticity to generate better quality images",
    )
    parser.add_argument(
        "--beta",
        type=float,
        default=0.0,
        help="the level of stochasticity"
    ...
    if size > 1 and args.compute_fid:
        ...
        for rank in range(size):
            ...
            p = Process(
                ...
                args=(global_rank, global_size, sample_and_test, args),
            )
            p.start()
            processes.append(p)
