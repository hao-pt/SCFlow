...
conda activate /lustre/scratch/client/vinai/users/quandm7/diffuser
cd /lustre/scratch/client/vinai/users/quandm7/cnf_flow
...
CUDA_VISIBLE_DEVICES={device} python -m torch.distributed.run --nnodes=1 --nproc_per_node={num_gpus} test_flow_latent_ddp.py --exp $EXP \
    --dataset $DATASET --batch_size 10 --epoch_id $EPOCH_ID \
    --image_size 1024 --f 8 --num_in_channels 4 --num_out_channels 4 \
    --nf 256 --ch_mult 1 1 2 2 4 4 --attn_resolution 16 8 4 --num_res_blocks 2 \
    --model_type $MODEL_TYPE \
    --num_classes 0 --label_dim 0. --label_dropout 0. \
...
###################################
slurm_file_path = f"/lustre/scratch/client/vinai/users/quandm7/cnf_flow/slurm_scripts/{exp}/run.sh"
slurm_output = f"/lustre/scratch/client/vinai/users/quandm7/cnf_flow/slurm_scripts/{exp}/"
