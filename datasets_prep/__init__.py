    ...
    if args.dataset == "cifar10":
        ...
        dataset = ImageNet(
            ...
            transform=transforms.Compose(
                [
                    transforms.Lambda(
                        lambda pil_image: center_crop_arr(pil_image, 256),
                    ),
                    transforms.RandomHorizontalFlip(),
                    transforms.ToTensor(),
                    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
        ...
        train_data = LSUN(
            root=args.datadir, classes=["bedroom_train"], transform=train_transform,
        )
        subset = list(range(120000))
        dataset = torch.utils.data.Subset(train_data, subset)
        ...
        dataset = LMDBDataset(
            root=args.datadir, name="celeba", train=True, transform=train_transform,
        )
        ...
        dataset = LMDBDataset(
            root=args.datadir, name="ffhq", train=True, transform=train_transform,
        )
    return dataset
