...
class PredictionHead(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, 256, kernel_size=4, stride=2)
        self.group_norm_1 = nn.GroupNorm(32, 256)
        # self.conv2 = nn.Conv2d(512, 256, kernel_size=4, stride=2)
        # self.group_norm_2 = nn.GroupNorm(32, 256)
        self.activation = nn.SiLU()

        # Output layer
        self.output_layer = nn.Sequential(nn.Linear(256 * 3 * 3, 1), nn.Sigmoid())

    def forward(self, x):
        x = self.conv1(x)  # from torch.Size([bsz, 1280, 8, 8]) to [bsz, 256, 3, 3]
...
def copy_weight_from_teacher(unet_stu, unet_tea):
    exclude = ["up_blocks", "conv_norm_out", "conv_act", "conv_out"]
    for k in unet_tea.state_dict().keys():
        flag = 0
        for ex in exclude:
            if k.startswith(ex):
                flag = 1
...
class SDDiscriminator(
    ...
    def __init__(
        ...
        self.disc_head = PredictionHead(block_out_channels[-1])
    ...
    def get_class_embed(
        self, sample: torch.Tensor, class_labels: torch.Tensor | None,
    ) -> torch.Tensor | None:
        class_emb = None
