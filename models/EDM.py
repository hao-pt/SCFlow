...
class CrossAttention(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.num_heads = (
            num_heads if num_heads is not None else query_channels // channels_per_head
        )
        self.use_mem_efficient_attn = use_mem_efficient_attn
        context_channels = (
            query_channels if context_channels is None else context_channels
        )
    ...
    def forward(self, x, context=None):
        q = self.q(x).reshape(
            x.shape[0] * self.num_heads, x.shape[1] // self.num_heads, -1,
        )
        context = x if context is None else context
        k = self.k(context).reshape(
            context.shape[0] * self.num_heads, x.shape[1] // self.num_heads, -1,
        )
...
class FeedForward(torch.nn.Module):
    def __init__(self, in_channels, out_channels=None, channel_mult=4, init=dict()):
        ...
        self.layer0 = Linear(
            in_features=in_channels, out_features=inner_channels, **init,
        )
        self.layer1 = Linear(
            in_features=inner_channels, out_features=out_channels, **init,
        )
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        return x.permute(0, 2, 1).view(
            x.size(0), -1, int(x.size(1) ** 0.5), int(x.size(1) ** 0.5),
        )
...
class DhariwalUNet(torch.nn.Module):
    def __init__(
        ...
        if self.use_context:
            self.map_label = (
                LabelEmbedder(label_dim, emb_channels, label_dropout)
                if label_dim
                else None
            )
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                ...
                self.dec[f"{res}x{res}_in1"] = resblock(
                    in_channels=cout, out_channels=cout, **block_kwargs,
                )
            else:
                self.dec[f"{res}x{res}_up"] = resblock(
                    in_channels=cout, out_channels=cout, up=True, **block_kwargs,
                )
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
    ...
    def forward_with_cfg(
        self, noise_labels, x, y=None, augment_labels=None, cfg_scale=1.0, **kwargs,
    ):
        """
        ...
        model_out = self.forward(
            noise_labels, combined, y, augment_labels, drop_half_label=True,
        )
