...
class BaseFlow:
    ...
    @torch.no_grad()
    def sample_ode_generative_stochastic(
        self, z1=None, N=None, use_tqdm=True, solver="euler", beta=0.0, model_kwargs={},
    ):
        assert solver in ["euler", "heun"]
        tq = tqdm if use_tqdm else lambda x: x
        if N is None:
            ...
            if len(z1.shape) == 2:
                if solver == "heun":
                    raise NotImplementedError(
                        "Heun's method not implemented for 2D data.",
                    )
                ...
                x0hat = z - vt * t.view(-1, 1, 1, 1)
                x0hat_list.append(x0hat)
            noise = ori_noise * (1 - beta) + beta * torch.randn_like(z)
            z = (
                x0hat.detach().clone() * (1.0 - t_next.view(-1, 1, 1, 1))
                + t_next.view(-1, 1, 1, 1) * noise
            )
            traj.append(z.detach().clone())
...
class ConsistencyFlow(RectifiedFlow):
    ...
    def get_train_tuple(self, z0=None, z1=None, t=None, eps=1e-5, model_kwargs={}):
        ...
        reflow_zt = (1 - t).view(-1, 1, 1, 1) * reflow_z0.detach() + t.view(
            -1, 1, 1, 1,
        ) * z1
