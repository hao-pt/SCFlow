...
from train_flow_latent import init_processes
...
def sample_and_test(rank, gpu, args):
    ...
    if args.dataset == 'cifar10':
        ...
        real_img_dir = 'pytorch_fid/celebahq_512_stat.npy'
    elif args.dataset == "celeba_1024":
        real_img_dir = 'pytorch_fid/celebahq_1024_stat.npy'
    else:
    ...
    def run_sampling(num_samples, generator=None):
        # x = generator.randn(num_samples, 4, args.image_size//8, args.image_size//8).to(device)
        x = torch.randn(num_samples, 4, args.image_size//8, args.image_size//8).to(device)
        if args.num_classes in [None, 1]:
            model_kwargs = {}
        else:
            # y = generator.randint(0, args.num_classes, (num_samples,), device=device)
            y = torch.randint(0, args.num_classes, (num_samples,), device=device)
